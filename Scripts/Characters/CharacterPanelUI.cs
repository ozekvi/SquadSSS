...
namespace Watermelon.SquadShooter
{
    public class CharacterPanelUI : MonoBehaviour
    {
        ...
        [SerializeField] TextMeshProUGUI rarityText;  // Добавлен для отображения редкости
        [SerializeField] Image characterBackImage;
        ...
        public void Initialise(Character character, UICharactersPanel charactersPanel)
        {
            ...
            rarityText.text = character.RarityData.Name;  // Устанавливаем текст редкости
            rarityText.color = character.RarityData.TextColor;  // Устанавливаем цвет текста редкости
            ...
            if (character.IsUnlocked())
            {
                ...
                ApplyGradientColorToImage(characterBackImage, character.RarityData.GradientColor);
        ...
        private void ApplyGradientColorToImage(Image image, Gradient gradient)
        {
            Texture2D texture = new Texture2D(1, 256);
            for (int y = 0; y < texture.height; y++)
            {
                texture.SetPixel(0, y, gradient.Evaluate((float)y / (texture.height - 1)));
            }
            texture.Apply();
